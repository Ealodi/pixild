    ...
    , ui(new Ui::Pixiv)
{
    ...
    ui->checkBox_2->setChecked(set.rankAuto);
    ...
    if (set.rankAuto)emit StartSearchRank("day",date);
}
...
void Pixiv::on_toolButton_clicked()
{
    ...
    case 2:
        page_tag = 1;
        TagY = 50;
        for (int i = 0;i < (int)tagPic.size();i++){
            tagPic[i]->setParent(NULL);
            delete tagPic[i];
        }
        tagPic.clear();
...
void ReadSetting(){
    ...
    if (file.open(QIODevice::ReadOnly))
    {
        ...
        set.rankAuto = obj.value("rankAuto").toBool();
        ...
        obj.insert("rankAuto",set.rankAuto);
...
void Pixiv::on_toolButton_4_clicked()
{
    ...
    set.rankAuto = ui->checkBox_2->isChecked();
    QJsonDocument document=QJsonDocument(obj);
    QByteArray array = document.toJson();
    file.write(array);
    file.close();
...
void Pixiv::TagSearch(QString word)
{
    ...
    QString url = base_url + "&page=" + QString::number(page_tag);
    QNetworkAccessManager *manager = new QNetworkAccessManager;
    //发出请求
    request->setUrl(QUrl(url));
    ...
    connect(manager,&QNetworkAccessManager::finished,[=](QNetworkReply* reply){
        ...
        for (int i = 0;i < AtPics.size();i++){
            ...
            label->SetPage(QString::number(AtPics.at(i).toObject().value("page_count").toInt()));
            label->setPixmap(Radius(QPixmap::fromImage(toux),width,height));
            label->move(491 / 2 - width / 2,TagY);
            label->show();
            ui->scrollAreaWidgetContents_5->setMinimumHeight(TagY + label->height() + 50);
            TagY += label->height() + 10;
            replys->deleteLater();

        }
        reply->deleteLater();
        ui->pushButton_4->setVisible(true);
        ui->pushButton_4->move(20,tagPic[tagPic.size() - 1]->pos().y() + tagPic[tagPic.size() - 1]->height());
